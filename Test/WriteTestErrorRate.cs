...
public class WriteTestErrorRate
{
    public static async Task Test(Ltfs.Ltfs lt)
    {
        ...
        if (lt.TapeDrive is not LTOTapeDrive drive)
        {
            Logger.Error("Tape drive is not an LTOTapeDrive. Cannot perform error rate test.");
            return;
        }
        ...
        if (diag != null)
        {
            await File.WriteAllBytesAsync("cm.bin", diag);
        }
